...
Updated the quickstart, operator guide, and extension-author guide to lead with the platform-specific release install bundle and `./install.sh`; retained manual placement as an explicit source-build fallback; and updated released-platform wording for Linux x86_64 and macOS arm64. The task verification greps and `git diff --check` passed. Implementation commit: `bc7cf8d`. Nothing remains for implementation.
