...
Implemented the fix specified by the Diagnosis Log entry (Diagnosis 1 — 2026-09-19). Confirmed the "red" state first: grepped and read `the-intern/docs/src/operator-guide/index.md` lines 1083-1197 and verified the step-4 `[[policy.action_rules]]` TOML block for the `email-triage` deployment walkthrough had a `bob worklog*` bash rule but no `bob task*` rule, matching the diagnosed isolated fault exactly. Cross-checked the reference shapes: the working `bob task*` rule at lines 336-340 ("The task board (`bob task`)" section) and the `bob worklog*` rule already in the step-4 block. Applied the minimal fix — added one `[[policy.action_rules]]` block (`tool = "bash"`, `arg_matchers = [{ field_path = "command", pattern = "bob task*" }]`) immediately before the `bob worklog*` rule, so the two "bob subcommand" admission rules sit adjacent, matching the diagnosis's requirement that it "sit within the step-4 block (alongside the existing `bob worklog*` rule)" and "mirror the exact shape" of the task-board rule. Did not touch surrounding prose: the "Cross-day continuity" paragraph (lines 1337-1344, corrected by T-211 in a prior task) already asserts the `bob task*` rule "is required for this workflow too," so no prose update was needed — the gap was purely the missing TOML rule itself, and adding prose beyond that would have been out of scope for this diagnosis's planned fix. Verified "green": confirmed via targeted line-range `awk`/`grep` (not a bare grep exit-status check, since the bug file itself flags that approach as insufficient — an existing unrelated `bob task*` match already lives in the "task board" section) that the new rule now appears at line 1192-1196, inside the step-4 block's line range and inside the "Deploying the `email-triage` scheduled job" section's overall range (977-1348). Ran `mdbook build the-intern/docs` (the `user-docs` CI check) and confirmed it completes cleanly with no errors — only the pre-existing, unrelated mdbook-mermaid version-mismatch informational warning appears, which is present on the unmodified file too. Committed the fix as a single commit (`640507d`, `docs(operator-guide): add bob task* rule to email-triage deploy policy`) on `bug/B-047-operator-guide-email-triage-policy-toml-missing-bob-task-rule`; `git diff --stat` against the pre-fix commit confirms exactly one file touched, 6 lines inserted, 0 deleted, matching the diagnosed fault's scope with no incidental changes. Nothing remains for a follow-up session on this bug; ready for review.
