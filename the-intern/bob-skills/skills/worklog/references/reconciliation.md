...
it compares the incoming entry's `Done` value against that item-identifier's
most recent entry already in today's file. If it matches exactly, nothing is
written — the entry already there records that state, so a second, identical
copy would add nothing. If it differs, or today's file has no entry yet for
that item-identifier, the incoming entry is appended as a new entry, however
...
and the same `Done` value, finds the entry already present and writes
nothing a second time. A caller that is unsure whether an
...
to match is not consulted. A `Done` value that differs from the item's last
entry, however slightly, is written as its own new entry.
