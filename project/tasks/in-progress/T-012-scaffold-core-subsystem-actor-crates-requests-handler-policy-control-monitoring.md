...
Validated the pending `the-intern/service/Cargo.lock` change and confirmed it only adds the expected local package stanzas for the three new T-012 crates: `requests-handler`, `policy-control`, and `monitoring` (each listing `async-trait`, `bob-core`, `tokio`, and `tracing`). No unrelated lockfile drift was present.

This lockfile update belongs on the task branch because T-012 introduced new workspace members and `cargo check --workspace` materializes those local package entries in `Cargo.lock`. Keeping the lockfile update in-branch prevents dirty-state churn when switching between `dev-agent` and the task branch and keeps verification reproducible.
