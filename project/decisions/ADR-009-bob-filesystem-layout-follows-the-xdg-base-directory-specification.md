...
Today bob already resolves most locations from XDG base directories
...
- The pi extension (`bob.ts`) has no bob-owned location — the operator installs
  it manually into pi's own search path (S-003). CR-003 needs a default path bob
  can pass to pi.
