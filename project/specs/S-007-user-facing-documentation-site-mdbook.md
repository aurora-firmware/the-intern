...
| 2026-06-23 | Noted that the User CLI guide (`bob chat`), the Operator/deployer guide (runtime layout / paths), and the Extension-author guide (install path) must reflect the XDG layout (ADR-009), the interactive-pi `bob chat` behaviour (CR-002, service-required), and the extension-by-path install (CR-003, `pi --extension`, default `~/.local/share/bob/extensions/bob.ts`) when authored. | Content reconciliation for CR-002 / CR-003 / ADR-009. | TBD (CR-002/CR-003 breakdown) |
